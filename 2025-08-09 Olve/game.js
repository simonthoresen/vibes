...
function setupWeaponSelection(isBossReward = false) {
    ...
    if (!isBossReward && Math.random() > 0.05) {
        weaponsList = weaponsList.filter(([id]) => !id.includes('DRAGON_'));
    }
        ...
        weapons.forEach(weapon => {
            ...
            if (checkbox.checked) {
                ...
                for (let i = 0; i < amount; i++) {
                    // Fix: Map display name to WEAPONS key
                    let weaponKey;
                    if (weapon === 'Piercing Bow') weaponKey = 'BOW';
                    else weaponKey = weapon.replace(/\s+/g, '_').toUpperCase();
