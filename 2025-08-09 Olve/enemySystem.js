import { ENEMY_TYPES, getCanvasWidth, getCanvasHeight } from './constants.js';
...
export class EnemySystem {
    ...
    getRandomSpawnPosition(enemyType) {
        ...
        switch(side) {
            case 0: // top
                ...
                break;
            case 1: // right
                x = getCanvasWidth() + enemyType.width;
                y = Math.random() * getCanvasHeight();
                break;
            case 2: // bottom
                x = Math.random() * getCanvasWidth();
                y = getCanvasHeight() + enemyType.height;
                break;
            case 3: // left
                x = -enemyType.width;
                y = Math.random() * getCanvasHeight();
    ...
    updateEnemyMovement(enemy) {
        ...
        if (Math.abs(dx) > getCanvasWidth() / 2) {
            dx = dx > 0 ? -(getCanvasWidth() - Math.abs(dx)) : getCanvasWidth() - Math.abs(dx);
        }
        
        // Check if wrapping around vertically would be shorter
        if (Math.abs(dy) > getCanvasHeight() / 2) {
            dy = dy > 0 ? -(getCanvasHeight() - Math.abs(dy)) : getCanvasHeight() - Math.abs(dy);
        }
    ...
    wrapEnemyPosition(enemy) {
        if (enemy.x < 0) enemy.x = getCanvasWidth();
        if (enemy.x > getCanvasWidth()) enemy.x = 0;
        if (enemy.y < 0) enemy.y = getCanvasHeight();
        if (enemy.y > getCanvasHeight()) enemy.y = 0;
    }
